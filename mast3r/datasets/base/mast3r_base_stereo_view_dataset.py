...
class MASt3RBaseStereoViewDataset(BaseStereoViewDataset):
    ...
    def generate_crops_from_pair(
        ...
        # )
        corres = (view1["corres"], view2["corres"], view1["valid_corres"])
        ...
        for i in range(2):
            ...
            corres = view["corres"]
            corres = np.hstack(
                (corres, np.ones((corres.shape[0], 1), dtype=corres.dtype))
            )
            matrix = np.linalg.inv(H)  # IDK why i need to invert the homography
            ...
            valid = np.logical_and(
                ...
                np.logical_and(0 <= corres[:, 1], corres[:, 1] < imsize[1] - 1),
            )
        ...
        valid_corres = views[0]["valid_corres"] & views[1]["valid_corres"]
    ...
    def __getitem__(self, idx):
        ...
        if self.n_corres > 0 and ("corres" not in view):
            raise RuntimeError("For dynamic scenes, corresondances need to be provided")
