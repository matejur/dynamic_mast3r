...
ROOT = "data"
SPLIT = "train"
OUTPUT_DIR = "data/panning_movi_e_point_tracking"
...
def get_num_to_sample(counts, max_seg_id, max_sampled_frac, tracks_to_sample):
    ...
    padding = max_seg_id - tf.shape(num_to_sample)[0]
    if padding > 0:
        num_to_sample = tf.concat(
            [num_to_sample, tf.zeros([padding], dtype=tf.int32)], axis=0
        )
    else:
        num_to_sample = num_to_sample[:max_seg_id]
...
def add_tracks(
    ...
    query_points, target_points, occluded, relative_depth, camera = track_points(
    ...
    video = tf.image.resize(tf.cast(video, tf.uint8), train_size)
    video.set_shape([num_frames, train_size[0], train_size[1], 3])

    size = tf.tensor_scatter_nd_update(
        tf.shape(depth), [[1], [2]], crop_window[2:4] - crop_window[0:2]
    )
    depth = tf.slice(depth, start, size)
    depth = tf.image.resize(tf.cast(depth, tf.float32), train_size)
    depth.set_shape([num_frames, train_size[0], train_size[1], 1])

    segmentations = tf.slice(segmentations, start, size)
    segmentations = tf.image.resize(segmentations, train_size, method="nearest")
    segmentations.set_shape([num_frames, train_size[0], train_size[1], 1])

    sensor_width = data["camera"]["sensor_width"]
    fx = data["camera"]["focal_length"] * 256 / sensor_width
    fy = data["camera"]["focal_length"] * 256 / sensor_width
    cx = 128
    cy = 128
    camera["intrinsics"] = tf.stack(
        [
            tf.stack([fx, 0.0, cx]),
            tf.stack([0.0, fy, cy]),
            tf.stack([0.0, 0.0, 1.0]),
    ...
    camera["depth_range"] = data["metadata"]["depth_range"]
...
def create_point_tracking_dataset(
    ...
    split="train",
    random_crop=False,
    tracks_to_sample=1024,
    sampling_stride=4,
    max_seg_id=25,
    max_sampled_frac=0.1,
    num_parallel_point_extraction_calls=8,
    snap_to_occluder=False,
    **kwargs,
):
...
def plot_tracks(rgb, points, occluded, trackgroup=None):
    ...
    for i in range(rgb.shape[0]):
        ...
        valid = np.logical_and(valid, ~occluded[:, i])
...
def main():
    ...
    ds = create_point_tracking_dataset(
        data_dir=ROOT, split=SPLIT, shuffle_buffer_size=None
    )

    path = os.path.join(OUTPUT_DIR, SPLIT)
    for i, data in enumerate(ds):
        os.makedirs(os.path.join(path, f"{i:05d}"), exist_ok=True)
        os.makedirs(os.path.join(path, f"{i:05d}", "rgbs"), exist_ok=True)
        os.makedirs(os.path.join(path, f"{i:05d}", "depths"), exist_ok=True)
        os.makedirs(os.path.join(path, f"{i:05d}", "masks"), exist_ok=True)

        for j in range(data["video"].shape[0]):
            cv2.imwrite(
                os.path.join(path, f"{i:05d}", "rgbs", f"{j:02d}.jpg"),
                cv2.cvtColor(data["video"][j], cv2.COLOR_RGB2BGR),
            )
            cv2.imwrite(
                os.path.join(path, f"{i:05d}", "depths", f"{j:02d}.png"),
                data["depth"][j, ..., 0].astype(np.uint16),
            )

            seg = Image.fromarray(data["segmentations"][j, ..., 0], "L")
            seg.save(os.path.join(path, f"{i:05d}", "masks", f"{j:02d}.png"))
        ...
        np.savez_compressed(os.path.join(path, f"{i:05d}", "tracks.npz"), **tracks)

        np.savez_compressed(
            os.path.join(path, f"{i:05d}", "camera.npz"), **data["camera"]
        )
